...
def build_resnet(version, config, num_classes, verbose=True, mask=False, batch_size=128):
    # config is useless
    version = desnet_versions[version]
    ...
    model = version["net"](
        block = version["block"], 
        nblocks = version["nblocks"], 
        growth_rate = version["growth_rate"],
...
if __name__ == '__main__':
    ...
    model_version = "densenet121Cifar"  # num_trainable_parameters should be 1000618 for cifar10
    model = build_resnet(model_version, "fanin", num_classes=10, mask=False, batch_size=128)
    # model = densenet121_cifar()
